...
public class UserServiceImpl implements UserService {
  ...
  public UserResponseDto deleteUserFromGroup(Integer userId, Integer groupId) {
    ...
    if (!groups.contains(group)) {
      throw new BadRequestException(
          ("User id=%s is not in the group id=%s".formatted(userId, groupId)));
    }
    groups.remove(group);
    userRepository.save(user);
    return userMapper.userToUserResponseDto(user);
  ...
  @Override
  public List<UserResponseDtoShort> getAllUsersByExample(UserRequestDto userRequestDto,
      PageRequest pageRequest) {
    Example<User> userExample = Example.of(userMapper.mapToEntity(userRequestDto),
        getExampleMatcher());
    return userMapper.userListToUserResponseDtoShortList(
